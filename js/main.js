...
function f_post(link, param){
	var div = param.split("#")[0];
...
function f_carregarAjaxList(lista){
    
	var aResult = lista.split("|");
	var link    = "http://funeral24h.com.br/";
	
	$.each(aResult, function(i, param){
		//alert( "Index #" + i + ": " + div );
		var div = param.split("#")[0];
	    //alert(div);
		switch(div) {
			case "div_menu":
				f_post(link+"ajax-menu.php",param);
                break;
			case "list-funerarias":
				f_post(link+"ajax-funerarias.php",param);
				break;				
			case "div_vejatambem":
				f_post(link+"ajaxvejatambem.php",param);
				break;
			case "div_footer":
				f_post(link+"ajaxfooter.php",param);
				break;
            case "div_form":
                f_post(link+"ajax-formulario.php",param);
                break;
            case "div_anuncios":
                f_post(link+"ajax-anuncios.php",param);
                break;
            case "div_filtro":
                f_post(link+"ajax-filtro-anuncio.php",param);
                break;    
			default:
				//default code block
		}
...
function f_carregarAjax(divResult, t, b){
	var dom = document.getElementById(divResult);
	if (dom != null) {
		$.post( "http://funeral24h.com.br/ajaxprod.php", { tipo: t, bairro: b })
			.done(function( data ) {
			$("#"+divResult).html(data);
		});
	}
...
function f_carregarAjax2(divResult, t, l){
    var dom = document.getElementById(divResult);
    if (dom != null) {
        $.post( "http://funeral24h.com.br/ajaxfooter.php", { tipo: t, local: l })
            .done(function( data ) {
            $("#"+divResult).html(data);
        });
    }
...
function f_carregarAjax3(divResult, t, l){
    var dom = document.getElementById(divResult);
    if (dom != null) {
        $.post( "http://funeral24h.com.br/ajaxvejatambem.php", { tipo: t, local: l })
            .done(function( data ) {
            $("#"+divResult).html(data);
        });
    }
...
function f_carregarAjax4(divResult){
    var dom = document.getElementById(divResult);
	var t = "";
	var l = "";
    if (dom != null) {
        $.post( "http://funeral24h.com.br/ajax-menu.php", { tipo: t, local: l })
            .done(function( data ) {
            $("#"+divResult).html(data);
        });
    }
...
function f_carregarHeader(divResult){

    var dom = document.getElementById(divResult);
    if(dom != null){
        $.post("http://funeral24h.com.br/ajax-header.php")
            .done(function(data){
...
function ajaxGetFunerarias(divResult, transpAereo,transpIntermunicipal,planoFuneral,atende24hs,tpMunicipal,tpParticular,uf){
    var dom = document.getElementById(divResult);
    if(dom != null){
		$.post("http://funeral24h.com.br/ajax-funerarias.php?transpAereo="+transpAereo+"&transpIntermunicipal="+transpIntermunicipal+"&planoFuneral="+planoFuneral+"&atende24hs="+atende24hs+"&tpMunicipal="+tpMunicipal+"&tpParticular="+tpParticular+"&uf="+uf)
        .done(function(data){
            $("#"+divResult).html(data);
        });
    }
...
function ajaxChangeCheckBox(divResult, classObj){
	
	var transpAereo= "";
	var transpIntermunicipal= "";
	var planoFuneral= "";
	var atende24hs= "";
	var tpMunicipal= "";
	var tpParticular= "";
	...
	if ($('input[id=transpAereo]').is(':checked')) { transpAereo = 1; }
	if ($('input[id=transpIntermunicipal]').is(':checked')) { transpIntermunicipal = 1; }
	if ($('input[id=planoFuneral]').is(':checked')) { planoFuneral = 1; }
	if ($('input[id=atende24hs]').is(':checked')) { atende24hs = 1; }
	if ($('input[id=tpMunicipal]').is(':checked')) { tpMunicipal = 1 }
	if ($('input[id=tpParticular]').is(':checked')) { tpParticular = 1 }
	
	tpParticular
	uf =  $("form[name=menu-lateral] input[class=ckUf]:checked").map(function() {
		return this.value;
	}).get();
		
	ajaxGetFunerarias(divResult, transpAereo,transpIntermunicipal,planoFuneral,atende24hs,tpMunicipal,tpParticular,uf);
}	

function envia_email()
{
    var res = "";
    $.post("http://funeral24h.com.br/ui-servico/contato_enviaemail.php", $("form[id='form_contato']").serialize() )
        .done(function (data){
            //$("#msg_sucesso").html(data);
            var res = data;
        });
    return res;
}
...
function valida_contato(){
    ...
    var email_de       = document.getElementById('email_de').value;
    ...
    if(email_de == ''){ msg += "- E-mail\n"; erro = 1; }else{
        var emailReg = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
        var emailaddressVal = email_de;
    ...
    //if(estado == 0){ msg += "- estado\n"; erro = 1;}
    //if(cidade == 0){msg +="- cidade\n"; erro = 1;}
    if(assunto == ''){ msg +="- Assunto\n";erro = 1;}
    if(mensagem == ''){ msg +="- Mensagem\n";erro = 1;}

    if(erro != 1){

        // envia email pelo ajax 
        envia_email();
        $("#msg_sucesso").html("Sua Mensagem foi enviada com Sucesso!!");
        $("#msg_sucesso").fadeIn('slow');
        $("#msg_sucesso").fadeOut(7000);

        // apaga os valores das inputs
...
function envia_anuncio(){
    ...
    if(erro != 1){
        ...
        $("form[id='form-anuncio']").ajaxForm({

            uploadProgress: function(){},

            success: function(data){
                $("#msg-anuncio").fadeIn();
                $("#msg-anuncio").html(data);
            },

            error: function(){},

            dataType: 'json',
            url: 'ajax-anuncios2.php',
            resetForm: true
    ...
    /*$.post("ajax-form-anuncio.php", $("form[id='form-anuncio']").serialize())
    .done(function(data){
        $("#msg-alert")
        .fadeIn()
        .html(data);
...
function update_cadastro(){
    ...
    $.post("ajax-update-cadastro.php", $("form[id='update-cadastro']").serialize())
    .done(function(data){
        $("#msg-update").fadeIn()
        .html(data).fadeOut(9999);
    });
