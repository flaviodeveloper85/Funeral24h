    ...
    var bootstrap_confirm_delete = function ( element, options )
    {
        ...
        this.onDelete = function ( event )
        {
            ...
            if ( undefined !== $( this ).attr( 'data-type' ) )
            {
                ...
                if(name == "esta conta")
                {
                    plugin.settings.heading = '<p style="font-size:25px">Excluir ' + name[ 0 ]/*.toUpperCase()*/ + name.substr( 1 ) + '</p>';
                    plugin.settings.message = '<p style="font-size:15px;text-align:center">Excluir ' + name + ' também irá remover possiveis anúncios publicados. Tem certeza?</p>';    
                }
                else
                {
                    plugin.settings.heading = '<p style="font-size:25px">Remover ' + name[ 0 ]/*.toUpperCase()*/ + name.substr( 1 ) + '</p>';
                    plugin.settings.message = '<p style="font-size:15px;text-align:center">Você tem certeza que deseja remover ' + name + '?</p>';    
                }
